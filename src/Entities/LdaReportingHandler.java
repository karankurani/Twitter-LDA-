...
public class LdaReportingHandler implements
	...
	public void fullMachineReport(LatentDirichletAllocation.GibbsSample sample,
			int maxWordsPerTopic, int maxTopicsPerDoc, boolean reportTokens) throws IOException {

		System.out.println("\nFull Report");
		PrintStream ps = new PrintStream(new File("C:/ALTData/TopicDetails.txt"));
		System.setOut(ps);
		int numTopics = sample.numTopics();
		int numWords = sample.numWords();
		int numDocs = sample.numDocuments();
